...
void main() {
  ...
  test('recupera syncing sem trocar identidade, payload ou operação', () async {
    ...
    await repository.recoverOrphanedSyncing(
  ...
  test('retry preserva client_request_id e payload semântico', () async {
    ...
    expect(
      after?.payload.items.single.productId,
      before?.payload.items.single.productId,
    );
  });

  test(
    'seleciona elegíveis mais antigos primeiro e respeita next_attempt_at',
    () async {
      await repository.register(
        localSaleId: 'older',
        clientRequestId: '11111111-2222-4333-8444-555555555555',
  ...
  test('payload corrompido vira permanente e não bloqueia o próximo', () async {
    ...
    final invalid = await (database.select(
      database.syncOutbox,
    )..where((row) => row.id.equals('bad'))).getSingle();
    expect(invalid.status, 'failed_permanent');
    expect(invalid.lastError, 'invalid_persisted_payload');
  });

  test(
    'aceite usa CAS de revisão e preserva intent em confirmação terminal',
    () async {
      const id = '11111111-2222-4333-8444-555555555555';
      await repository.register(
        localSaleId: id,
  ...
  test(
    '403 bloqueia sem apagar token, identidade, payload ou operação',
    () async {
      const id = '11111111-2222-4333-8444-555555555555';
      await repository.register(
        localSaleId: id,
  ...
  test(
    'cancelamento controlado devolve para pending sem alterar a operação',
    () async {
      const id = '11111111-2222-4333-8444-555555555555';
      await repository.register(
        localSaleId: id,
  ...
  test(
    'restart recupera syncing com a mesma identidade, payload e operação',
    () async {
  ...
  test(
    'stock_proposal_changed substitui proposta/token e exige nova revisão',
    () async {
      const id = '11111111-2222-4333-8444-555555555555';
      await repository.register(
        localSaleId: id,
      ...
      await repository.markRequiresAcceptance(
        id,
        remoteIntentId: 'intent-1',
        proposalJson: '{"revision":2}',
        confirmationToken: 'token-2',
        now: now,
      );

      final changed = await repository.read(id);
      expect(changed?.status, SaleSyncStatus.requiresAcceptance);
      expect(changed?.proposalRevision, 2);
      expect(changed?.confirmationToken, 'token-2');
      expect(
        await repository.queueForConfirmation(
          id,
          expectedProposalRevision: 1,
  ...
  test(
    'restart preserva proposta, token e revisão para aceite humano',
    () async {
      ...
      try {
        ...
        await fileRepository.markRequiresAcceptance(
          id,
          remoteIntentId: 'intent-1',
          proposalJson: '{"proposed_items":[]}',
        ...
        fileRepository = DriftSalesRepository(fileDatabase);
        final recovered = await fileRepository.read(id);
        expect(recovered?.status, SaleSyncStatus.requiresAcceptance);
        expect(recovered?.remoteIntentId, 'intent-1');
        expect(recovered?.confirmationToken, 'fresh-token');
        expect(recovered?.proposalRevision, 1);
        expect(
          await fileRepository.queueForConfirmation(
            id,
            expectedProposalRevision: 1,
            now: now,
          ),
