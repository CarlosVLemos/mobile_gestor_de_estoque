...
void main() {
  setUpAll(() async {
    ...
    for (final asset in const [
      ...
      'assets/fonts/instrument_sans/InstrumentSans-Bold.ttf',
    ]) {
      fonts.addFont(rootBundle.load(asset));
    }
    await fonts.load();
  ...
  group('Spec 014 visual goldens (390x844)', () {
    ...
    Future<void> golden(Finder finder, String name) =>
        expectLater(finder, matchesGoldenFile('goldens/$name.png'));

    testWidgets('G-001 Shell light', (t) async {
      await pump(t, _shell(0), AppTheme.light);
      await golden(find.byType(AppShellScaffold), 'shell_claro');
    });
    testWidgets('G-002 Shell dark', (t) async {
      await pump(t, _shell(1), AppTheme.dark);
      await golden(find.byType(AppShellScaffold), 'shell_escuro');
    });
    testWidgets('G-003 Login light', (t) async {
      await pump(t, const LoginPage(), AppTheme.light, [
        authControllerProvider.overrideWith(_GoldenLoginController.new),
      ]);
      await golden(find.byType(LoginPage), 'login_claro');
    });
    testWidgets('G-004 Login dark', (t) async {
      await pump(t, const LoginPage(), AppTheme.dark, [
        authControllerProvider.overrideWith(_GoldenLoginController.new),
      ]);
      await golden(find.byType(LoginPage), 'login_escuro');
    });
    testWidgets('G-005 Change Password light', (t) async {
      await pump(t, const ChangePasswordPage(), AppTheme.light, [
        authControllerProvider.overrideWith(_GoldenPasswordController.new),
      ]);
      await golden(find.byType(ChangePasswordPage), 'change_password_claro');
    });
    testWidgets('G-006 Change Password dark', (t) async {
      await pump(t, const ChangePasswordPage(), AppTheme.dark, [
        authControllerProvider.overrideWith(_GoldenPasswordController.new),
      ]);
      await golden(find.byType(ChangePasswordPage), 'change_password_escuro');
    });
    testWidgets('G-007 Dashboard light', (t) async {
      await pump(t, const DashboardPage(), AppTheme.light, [
        dashboardRepositoryProvider.overrideWithValue(
          const _DashboardFixtureRepository(),
        ),
      ]);
      await golden(find.byType(DashboardPage), 'dashboard_claro');
    });
    testWidgets('G-008 Dashboard dark', (t) async {
      await pump(t, const DashboardPage(), AppTheme.dark, [
        dashboardRepositoryProvider.overrideWithValue(
          const _DashboardFixtureRepository(),
        ),
      ]);
      await golden(find.byType(DashboardPage), 'dashboard_escuro');
    });
    testWidgets('G-009 Catalog light', (t) async {
      await pump(t, const CatalogPage(), AppTheme.light, [
        catalogRepositoryProvider.overrideWithValue(
          const _CatalogFixtureRepository(),
        ),
      ]);
      await golden(find.byType(CatalogPage), 'catalogo_claro');
    });
    testWidgets('G-010 Catalog dark', (t) async {
      await pump(t, const CatalogPage(), AppTheme.dark, [
        catalogRepositoryProvider.overrideWithValue(
          const _CatalogFixtureRepository(),
        ),
      ]);
      await golden(find.byType(CatalogPage), 'catalogo_escuro');
    });
    testWidgets('G-011 Sales New light', (t) async {
      await pump(t, const SalesPage(), AppTheme.light, _salesOverrides());
      await t.ensureVisible(find.text('Registrar venda'));
      await t.pumpAndSettle();
      await golden(find.byType(SalesPage), 'vendas_nova_claro');
    });
    testWidgets('G-012 Sales New dark', (t) async {
      await pump(t, const SalesPage(), AppTheme.dark, _salesOverrides());
      await t.ensureVisible(find.text('Registrar venda'));
      await t.pumpAndSettle();
      await golden(find.byType(SalesPage), 'vendas_nova_escuro');
    });
    testWidgets('G-013 Sales History light', (t) async {
      await pump(
        t,
        const SalesPage(),
        AppTheme.light,
        _salesOverrides(history: true),
      );
      await t.tap(find.byKey(const ValueKey('sales-segment-history')));
      await t.pumpAndSettle();
      await golden(find.byType(SalesPage), 'vendas_historico_claro');
    });
    testWidgets('G-014 Sales History dark', (t) async {
      await pump(
        t,
        const SalesPage(),
        AppTheme.dark,
      ...
      await t.tap(find.byKey(const ValueKey('sales-segment-history')));
      await t.pumpAndSettle();
      await golden(find.byType(SalesPage), 'vendas_historico_escuro');
    });
    testWidgets('G-015 More light', (t) async {
      await pump(t, const MorePage(), AppTheme.light, _settingsOverrides());
      await golden(find.byType(MorePage), 'mais_claro');
    });
    testWidgets('G-016 More dark', (t) async {
      await pump(t, const MorePage(), AppTheme.dark, _settingsOverrides());
      await golden(find.byType(MorePage), 'mais_escuro');
    });
    testWidgets('G-017 Account Company light', (t) async {
      await pump(
        t,
        const OperationalContextPage(),
        AppTheme.light,
        _settingsOverrides(),
      );
      await golden(find.byType(OperationalContextPage), 'conta_empresa_claro');
    });
    testWidgets('G-018 Account Company dark', (t) async {
      await pump(
        t,
        const OperationalContextPage(),
        AppTheme.dark,
      ...
      await golden(find.byType(OperationalContextPage), 'conta_empresa_escuro');
    });
  });
}

AppShellScaffold _shell(int index) => AppShellScaffold(
  currentIndex: index,
  destinations: const [
    AppBottomNavigationDestination(label: 'Painel', icon: AppIcons.dashboard),
    AppBottomNavigationDestination(label: 'Produtos', icon: AppIcons.products),
    AppBottomNavigationDestination(label: 'Vendas', icon: AppIcons.sales),
    AppBottomNavigationDestination(label: 'Mais', icon: AppIcons.more),
  ],
  onSelect: (_) {},
  body: const Center(child: Text('Visão operacional')),
);

class _GoldenLoginController extends AuthController {
  @override
  AuthState build() => const AuthState(status: AuthStatus.unauthenticated);
}

class _GoldenPasswordController extends AuthController {
  @override
  AuthState build() =>
      const AuthState(status: AuthStatus.passwordChangeRequired);
}

class _GoldenSalesController extends SalesController {
  @override
  SalesState build() => SalesState(
    clients: const [],
    products: const [],
    selectedClient: const SaleClientOption(
      id: '201',
      name: 'Cliente Norte',
      code: 'CLI-201',
      city: 'Belém - PA',
...
List<Override> _salesOverrides({bool history = false}) => [
  shellProfileProvider.overrideWithValue(_profile),
  salesControllerProvider.overrideWith(_GoldenSalesController.new),
  persistedSalesProvider.overrideWith(
    (ref) => Stream.value(history ? _history : const []),
  ),
];
final _history = [
  PersistedSaleSummary(
    localSaleId: 'confirmed',
  ...
  PersistedSaleSummary(
    ...
    createdAt: DateTime(2026, 9, 21, 16),
    status: SaleSyncStatus.requiresAcceptance,
    itemCount: 1,
    proposalRevision: 2,
    proposalJson: '{"proposal":true}',
  ),
];
...
final _profile = ShellProfile(
  ...
  tenantName: 'Arara Centro Logístico',
